...
namespace DtxUpload {
	...
	public class ServerConnector {
		...
		public ServerConnector(ServerConnectorServerInformation server, ServerConnectorUserInformation user, object tag) {
			App.Settings.AddValueChangedEvent("connector.max_download_kbps", () => {
				if (App.Settings.Get<bool>("connector.max_download_enabled", false) == true) {
					throttle_bytes_down = App.Settings.Get<long>("connector.max_download_kbps");
			...
			App.Settings.AddValueChangedEvent("connector.max_upload_kbps", () => {
				if (App.Settings.Get<bool>("connector.max_upload_enabled", false) == true) {
					throttle_bytes_up = App.Settings.Get<long>("connector.max_upload_kbps");
			...
			App.Settings.AddValueChangedEvent("connector.max_download_enabled", () => {
				if (App.Settings.Get<bool>("connector.max_download_enabled", false) == false) {
					throttle_bytes_down = ThrottledStream.Infinite;
				}
			});

			App.Settings.AddValueChangedEvent("connector.max_upload_enabled", () => {
				if (App.Settings.Get<bool>("connector.max_upload_enabled", false) == false) {
					throttle_bytes_up = ThrottledStream.Infinite;
				}
			});
		}
